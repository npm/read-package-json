...
function bins (file, data, cb) {
  ...
  var bin = data.directories && data.directories.bin
  if (data.bin || !bin) {
    return cb(null, data)
  }
...
function githead (file, data, cb) {
  ...
  var dir = path.dirname(file)
  var headFile = path.resolve(dir, '.git/HEAD')
  fs.readFile(headFile, 'utf8', function (headFileErr, headData) {
    ...
    var headRef = headData.replace(/^ref: /, '').trim()
    var headRefFile = path.resolve(dir, '.git', headRef)
